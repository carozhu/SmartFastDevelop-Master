 ...
 * 不带优先级的线程池
 */
public class CachedThreadPool implements ICachedThreadPool {
    ...
    private CachedThreadPool() {
        mExecutorService = new ThreadPoolExecutor(THREADS_CORE_COUNT, THREADS_MAX_COUNT, THREADS_KEEP_ALIVE_TIME, THREADS_KEEP_ALIVE_UNIT, new LinkedBlockingQueue<Runnable>());
    ...
    @Override
    public <T> Future<T> submit(final Callable<T> task, final Runnable taskSuc, final Runnable taskFail, final Runnable taskCancel) {
